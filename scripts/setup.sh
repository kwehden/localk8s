...
register_local_hostname_env() {
  local local_hostname="${LOCAL_HOSTNAME:-}"

  if [[ -z "${local_hostname}" && -f "${LOCAL_ENV_FILE}" ]]; then
    # shellcheck disable=SC1090
...
# Local host-specific overrides for localk8s scripts.
# Generated by scripts/setup.sh. Edit only when you intentionally want custom values.
LOCAL_HOSTNAME="${local_hostname}"
EOF

  log "registered LOCAL_HOSTNAME in ${LOCAL_ENV_FILE}"
